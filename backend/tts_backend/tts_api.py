...
from backend.tts_backend.emotion import cosyvoice
...
try:
    ...
    # 尝试不同的路径找到参考音频
    possible_paths = [
        current_file.parents[2] / 'resource' / 'voice' / "NeZha''.mp3",  # 项目根目录
...
try:
    ...
    cosyvoice = CosyVoice2(
        ...
        load_trt=False,
        fp16=False
    ) if cosyvoice is None else cosyvoice

    logging.info("CosyVoice2 initialized successfully")
except Exception as e:
    logging.error(f"Error initializing CosyVoice2: {str(e)}")
    logging.error(traceback.format_exc())
    sys.exit(1)
